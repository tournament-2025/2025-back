...
route.get("/match/:class/", async (c) => {
  const target = parseInt(await c.req.param("class"))
  const res = []

  try {
    // let data1 = await prisma.match.findFirst({ where: { id: id as string } });
    const data = await prisma.match.findMany()
    data.forEach((data1) => {
